...
_Status: draft v1 · 2026-09-25 · Source of truth for **what** to build. `intent.md` holds the **why**._
...
| M1 | Data layer: Drizzle + SQLite, CV Zod schema, import of `sample_data/` (36 CVs), revisions | Import test proves all 36 CVs validate and round-trip |
...
| M3 | Search: FTS5 full-text search plus facet filters, results grouped by person | Search E2E: "Scrum Kanervapankki" finds p10's versions |
...
- Deployment: container, managed Postgres, secrets from a vault, HTTPS, backups.
...
| DB | SQLite (`better-sqlite3`) + Drizzle ORM | 13.0.x / 0.45.x (drizzle-kit 0.31.x) | Zero-ops local DB, FTS5 built in, parameterized by default. Traffic is ≤ 40 users (see §15 D1) |
...
All ids are UUIDv7 (time-sortable, not guessable) except the imported `person.legacyId`
(`p01`…). Timestamps are ISO-8601 UTC text.
...
| `cv` | `id`, `personId`, `variant` (e.g. `default`, `PM`), `title`, `currentRevisionId`, `reviewedAt`, `createdAt`, `updatedAt`, `archivedAt` | Unique (`personId`, `variant`) among non-archived |
| `cv_revision` | `id`, `cvId`, `number` (1..n), `data` (JSON text), `dataSha256`, `source` (`import`/`manual`/`ai`/`restore`/`duplicate`), `message`, `authorId`, `aiMessageId`, `createdAt` | **Append-only.** SQLite triggers `RAISE(ABORT)` on UPDATE/DELETE; only `person.erase` drops the trigger inside its transaction |
| `tag`, `cv_tag` | `tag.name` unique (case-insensitive), `color` | Curated labels such as "Security clearance", "Available Q4" |
| `cv_facet` | `cvId`, `kind` (`skill`/`skillCategory`/`industry`/`role`/`client`/`keyword`/`variant`), `value`, `valueNorm` | Rebuilt from the current revision on every save |
| `cv_search` | FTS5 virtual table (§7.2) | Rebuilt from the current revision on every save |
| `ai_conversation` | `id`, `cvId`, `userId`, `createdAt` | One per CV per user per session |
| `ai_message` | `id`, `conversationId`, `role`, `content`, `proposal` (JSON), `status` (`proposed`/`applied`/`rejected`/`invalid`/`failed`), `inputTokens`, `outputTokens`, `createdAt` | Kept for 90 days, then purged by a job ([Later]; the MVP documents it) |
| `audit_event` | `id`, `at`, `actorId`, `action`, `targetType`, `targetId`, `outcome` (`allowed`/`denied`/`error`), `requestId`, `meta` (JSON, **never CV content**) | Append-only (trigger). Logs reads, exports, edits, AI calls, logins and denials |
...
- SQLite pragmas: `journal_mode=WAL`, `foreign_keys=ON`, `busy_timeout=5000`,
  `secure_delete=ON` (so erased personal data is overwritten on disk).
- The DB file lives in `./data/` (gitignored), with file mode `0600`.
...
from `index.json`. It's idempotent (upsert on `legacyId` + `variant`). Anything that fails
...
- A SQLite **FTS5** table `cv_search(cv_id UNINDEXED, name, label, variant, keywords,
  skills, clients, industries, roles, body)` with `tokenize = "unicode61 remove_diacritics 2"`,
  so "Makinen" matches "Mäkinen".
- Ranking: `bm25(cv_search, 0, 10, 6, 4, 5, 5, 4, 3, 3, 1)`. There's one weight per column,
  in table order, and the `cv_id UNINDEXED` column still takes the first weight slot (0), so a match on the name ranks above a match on a project description.
- **FTS injection prevention (A05):** raw user input never goes into `MATCH`. The query
  string is split into ≤ 10 terms of ≤ 64 characters. Each term is wrapped as an FTS5 string
  literal (`"term"*`, with inner quotes doubled) and the terms are joined with implicit AND.
  All values are bound parameters. Unit tests cover `"`, `*`, `NEAR`, `OR`, `-`, `:`,
  `^` and column filters.
- Facet filters query `cv_facet` with `valueNorm IN (?)`. Values in the same facet are ORed;
...
| **A02 Security Misconfiguration** | Security headers middleware (§9.2). Env validated with Zod at boot (fails fast, no defaults for secrets). Dev tooling isn't reachable in prod builds. `X-Powered-By` removed. Errors are generic. Sign-up disabled. The DB file is `0600` and outside the web root |
| **A03 Software Supply Chain Failures** | pnpm with a committed lockfile, `minimumReleaseAge: 1440` (24 h), `onlyBuiltDependencies` allowlist (`better-sqlite3`, `esbuild`, `playwright`), exact versions, `pnpm audit --audit-level=high` in `pnpm verify`, minimal dependencies (§4), no runtime CDNs, self-hosted fonts |
| **A04 Cryptographic Failures** | scrypt password hashing (Better Auth). Session tokens ≥ 256-bit random, stored hashed. `BETTER_AUTH_SECRET` ≥ 32 bytes from `.env.local`. HTTPS + HSTS when deployed. SQLite `secure_delete`. Disk encryption of the dev machine assumed; full DB encryption [Later] |
| **A05 Injection** | Drizzle parameterized queries only; `sql.raw` banned by lint. FTS5 query builder escapes every term. React auto-escaping; `dangerouslySetInnerHTML` banned by lint. Strict CSP with nonces. Zod on every input. JSON patch path allowlist. Content-Disposition file names sanitized. The PDF HTML is built by React, never by string concatenation |
...
| **A10 Mishandling of Exceptional Conditions** | `Result` types for expected failures. A global error boundary plus server error middleware that **fails closed** (on any authz or validation error → deny). No empty `catch` (lint). Timeouts on Chromium, the LLM and the DB busy timeout. Every transaction rolls back on error |
...
| Integration (~20%) | Vitest + real SQLite (in-memory, migrated per test file) | Services and server functions with the real DB; Chromium PDF render; import | Save with a stale `baseRevisionId` → 409; import of all 36 samples; the PDF contains tags and the attachment |
...
- `pnpm verify` = typecheck + lint + unit/integration tests + `pnpm audit` + secret scan +
  build. E2E runs with `pnpm test:e2e`.
...
```
DATABASE_PATH=./data/cvbank.sqlite
...
startup and refuses to start when they're invalid.
...
| `pnpm test:e2e` | Playwright (starts the app with a fresh DB and `AI_PROVIDER=fake`) |
...
`registry.npmjs.org` and the Playwright browser CDN. The `anthropic` provider needs
...
|---|---|---|---|
| D1 | **SQLite + Drizzle** instead of OracleDB | Local-only, ≤ 40 users, zero ops, FTS5 built in, fits five hours. Drizzle keeps a later move to Postgres or Oracle mechanical | Cloud deployment |
