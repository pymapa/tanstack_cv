...
async function readJson<T>(relativePath: string): Promise<T> {
  return JSON.parse(await readFile(path.join(DATA_DIR, relativePath), 'utf8')) as T
}
 ...
 */
export async function readCv(file: string, people?: Array<IndexPerson>): Promise<Cv | null> {
  const listed = (people ?? (await readPeople())).some((p) => p.versions.some((v) => v.file === file))
